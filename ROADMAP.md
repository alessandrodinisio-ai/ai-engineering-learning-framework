...
| 02 | [词袋、TF-IDF 与文本表示](phases/05-nlp-foundations-to-advanced/02-bag-of-words-tfidf) | ✅ | ~75 min |
...
| 17 | [聊天机器人 — 从规则驱动到神经网络](phases/05-nlp-foundations-to-advanced/17-chatbots-rule-to-neural) | ✅ | ~75 min |
...
| 02 | [频谱图、梅尔标度与音频特征](phases/06-speech-and-audio/02-spectrograms-mel-features) | ✅ | ~45 min |
...
| 08 | [声音克隆与音色转换](phases/06-speech-and-audio/08-voice-cloning-conversion) | ✅ | ~75 min |
...
| 03 | [GAN — 生成器对判别器](phases/08-generative-ai/03-gans-generator-discriminator/) | ✅ | ~75 min |
...
| 09 | [图像修复、扩展与编辑](phases/08-generative-ai/09-inpainting-outpainting-editing/) | ✅ | ~75 min |
...
| # | Lesson | Status | Est. |
|---|--------|--------|------|
| 01 | [Vision Transformer 与图块-token 原语](phases/12-multimodal-ai/01-vision-transformer-patch-tokens) | ✅ | ~120 min |
| 02 | [CLIP 与对比式视觉语言预训练](phases/12-multimodal-ai/02-clip-contrastive-pretraining) | ✅ | ~180 min |
...
| 07 | [开源权重 VLM 配方：真正重要的是什么](phases/12-multimodal-ai/07-open-weight-vlm-recipes) | ✅ | ~180 min |
...
| 11 | [Chameleon 与早融合纯 token](phases/12-multimodal-ai/11-chameleon-early-fusion-tokens) | ✅ | ~180 min |
| 12 | [Emu3 用下一 token 预测做生成](phases/12-multimodal-ai/12-emu3-next-token-for-generation) | ✅ | ~120 min |
...
| 16 | [MIO 任意到任意流式处理](phases/12-multimodal-ai/16-mio-any-to-any-streaming) | ✅ | ~120 min |
| 17 | [视频语言时序定位](phases/12-multimodal-ai/17-video-language-temporal-grounding) | ✅ | ~180 min |
...
| 20 | [全模态模型：Thinker-Talker](phases/12-multimodal-ai/20-omni-models-thinker-talker) | ✅ | ~180 min |
...
| 15 | [投票、自洽性与辩论拓扑](phases/16-multi-agent-and-swarms/15-voting-debate-topology/) | ✅ | ~75 min |
| 16 | [协商与议价](phases/16-multi-agent-and-swarms/16-negotiation-bargaining/) | ✅ | ~75 min |
| 17 | [生成式智能体与涌现式仿真](phases/16-multi-agent-and-swarms/17-generative-agents-simulation/) | ✅ | ~75 min |
...
| 22 | [生产级扩展 — 队列、检查点、持久性](phases/16-multi-agent-and-swarms/22-production-scaling-queues-checkpoints/) | ✅ | ~75 min |
| 23 | [故障模式 — MAST、群体思维、单一文化、级联](phases/16-multi-agent-and-swarms/23-failure-modes-mast-groupthink/) | ✅ | ~75 min |
...
| 08 | [前沿模型中的上下文阴谋](phases/18-ethics-safety-alignment/08-in-context-scheming-frontier-models) | ✅ | ~60 min |
| 09 | [对齐伪装](phases/18-ethics-safety-alignment/09-alignment-faking) | ✅ | ~60 min |
| 10 | [AI 控制 — 颠覆之下仍保安全](phases/18-ethics-safety-alignment/10-ai-control-subversion) | ✅ | ~75 min |
...
| 17 | [WMDP 与两用能力评估](phases/18-ethics-safety-alignment/17-wmdp-dual-use-evaluation) | ✅ | ~60 min |
...
| 20 | [偏见与表征伤害](phases/18-ethics-safety-alignment/20-bias-representational-harm) | ✅ | ~60 min |
...
| 26 | [模型卡、系统卡与数据集卡](phases/18-ethics-safety-alignment/26-model-system-dataset-cards) | ✅ | ~60 min |
| 27 | [数据溯源与训练数据治理](phases/18-ethics-safety-alignment/27-data-provenance-training-governance) | ✅ | ~60 min |
...
| 02 | [面向代码库的 RAG (跨仓库语义搜索)](phases/19-capstone-projects/02-rag-over-codebase) | ✅ | ~30 hr |
...
| 05 | [自主研究智能体 (AI-Scientist 级)](phases/19-capstone-projects/05-autonomous-research-agent) | ✅ | ~40 hr |
| 06 | [面向 Kubernetes 的 DevOps 排障智能体](phases/19-capstone-projects/06-devops-troubleshooting-agent) | ✅ | ~30 hr |
| 07 | [端到端微调流水线](phases/19-capstone-projects/07-end-to-end-fine-tuning-pipeline) | ✅ | ~35 hr |
| 08 | [生产级 RAG 聊天机器人 (受监管垂直领域)](phases/19-capstone-projects/08-production-rag-chatbot) | ✅ | ~30 hr |
...
| 13 | [带注册中心与治理的 MCP Server](phases/19-capstone-projects/13-mcp-server-with-registry) | ✅ | ~25 hr |
| 14 | [推测解码推理服务器](phases/19-capstone-projects/14-speculative-decoding-server) | ✅ | ~30 hr |

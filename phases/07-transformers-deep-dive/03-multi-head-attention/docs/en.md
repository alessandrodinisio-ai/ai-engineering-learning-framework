...
- [Raschka, *Build a Large Language Model (From Scratch)* — Ch. 3 main chapter code](https://github.com/rasbt/LLMs-from-scratch/tree/main/ch03/01_main-chapter-code) — the clearest step-by-step PyTorch derivation of causal + multi-head attention starting from a single head. Follow the order `ch03.ipynb` → `multihead-attention.ipynb`.
- [Raschka, Ch. 3 bonus — `mha-implementations.ipynb`](https://github.com/rasbt/LLMs-from-scratch/blob/main/ch03/02_bonus_efficient-multihead-attention/mha-implementations.ipynb) — six MHA variants benchmarked side-by-side (manual, `nn.MultiheadAttention`, `scaled_dot_product_attention`, FlexAttention, Flash). Best source for "how much does each variant cost in real wall-clock."
- [Raschka, Ch. 4 bonus — `04_gqa`](https://github.com/rasbt/LLMs-from-scratch/tree/main/ch04/04_gqa) — GQA wired into a full tiny-GPT, with parameter and memory tables you can compare against this lesson's MHA.
- [Raschka, Ch. 4 bonus — `05_mla`](https://github.com/rasbt/LLMs-from-scratch/tree/main/ch04/05_mla) — Multi-head Latent Attention implementation referenced in the DeepSeek-V3 discussion.
- [Karpathy, nanoGPT `model.py::CausalSelfAttention`](https://github.com/karpathy/nanoGPT/blob/master/model.py) — the single cleanest production reference implementation of MHA on GPU.
